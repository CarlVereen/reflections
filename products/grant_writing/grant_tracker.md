# 📋 Grant Tracker — Howard's Orchard LLC
### One page, every grant, at a glance. Updated 2026-07-15.

**Status key:** ☐ not started · ⏳ in progress · ✅ submitted · 🏆 won · ❌ not selected
**Update the Status column as you go.** Sorted by "do this first."

---

## 🔥 DO NOW (open + time-sensitive)

| Status | Grant | Amount | Who applies | Deadline | Next step |
|---|---|---|---|---|---|
| ☐ | **Amber Grant** | $10,000/mo | **Ashley** (51%) | Rolling monthly | Paste essay, pay $15, submit → `email_to_ashley.md` |
| ☐ | **Galaxy Grants** | $4,250 | Either | **Jul 31, 2026** | Paste answers at hiddenstar.org → `galaxy_grant_entry.md` |
| ☐ | **Venmo Small Business Grant** | $20,000 | Either | Check round | Qualifies (under $50k revenue) |
| ☐ | **Verizon Digital Ready** | $10,000/mo | Either | Monthly Jun–Dec (one app) | Apply once at digitalready.verizonwireless.com |
| ☐ | **NRCS EQIP (water)** | 50–90% of cost | Carl | Ongoing (ask ranking date) | Call Pima NRCS → `nrcs_call_script.md` |
| ☐ | **AZ Organic Cost-Share** | 75% up to $750 | Either | Dec 31, 2026 | Call Robert Nick (602) 542-0439 |
| ☐ | **NASE Growth Grant** | up to $4,000 | Either | Year-round (members) | Join NASE (~$120/yr), apply for equipment |
| ☐ | **Freed Fellowship** | $500/mo + $2,500/yr | **Ashley** | Rolling monthly ($19) | Apply at thefreedfellowship.com (do with Amber) |
| ☐ | **The Pollination Project** | $1,000 | Either | Rolling (daily awards) | Frame community/education angle, apply |

## 📝 FREE SIGNUPS (do once, they find grants for you)

| Status | Platform | Why | Next step |
|---|---|---|---|
| ☐ | **Hello Alice** | Matches ag + small-biz grants | Free account at helloalice.com |
| ☐ | **IFundWomen** | Women's grant rounds | Universal app at ifundwomen.com |

## ⏳ BIG ONES — WATCH FOR NEXT CYCLE (we prep now)

| Status | Grant | Amount | Fit | When |
|---|---|---|---|---|
| ☐ | **USDA Business Builder Award** (SW Regional Food Business Center / Local First AZ) | **up to $100,000** | ⭐ Value-added AZ farm under $350k sales = you | Check status now at localfirstaz.com / goodfoodfinderaz.com |
| ☐ | **USDA VAPG** | up to ~$50k (simplified) | ⭐ Jams/cider/pies value-added | ~early 2027 · drafts in `vapg/` |
| ☐ | **AFT Brighter Future Fund** | up to $10,000 | Equipment/infrastructure | Next cycle 2026 · farmland.org/brighterfuture |
| ☐ | **FruitGuys Community Fund** | up to $5,000 | Water/solar/pollinator project | Reopens ~late 2026 · fruitguyscommunityfund.org |
| ☐ | **Western SARE Farmer/Rancher** | ~$25–30k | Elderberry/mulberry + water trials | Annual cycle · western.sare.org |
| ☐ | **REAP** | $1.5k–$500k | Solar irrigation / cold storage | Rolling quarterly |
| ☐ | **AZ RFSI** (Resilient Food Systems) | competitive | Food-processing infrastructure | Next cycle · agriculture.az.gov |
| ☐ | **AZ Specialty Crop Block Grant** | varies | Industry/research angle | ~March 2027 |
| ☐ | **USDA FMPP** | up to $500,000 | ⭐ Agritourism / U-pick direct sales | ~2027 · ams.usda.gov/services/grants/fmpp |
| ☐ | **AFT / FruitGuys / Xerces (pollinator)** | $5–10k | Elderberry/mulberry + habitat | Next cycles · pairs w/ water project |
| ☐ | **AZ Water Conservation Grant Fund (WIFA)** | varies | ⭐ Ag irrigation efficiency (water system) | Check cycle · azwifa.gov |
| ☐ | **AZ ag water-conservation TAX CREDIT** | tax credit | Money back on water-system install | Tell accountant (not an application) |
| ☐ | **AgWest Farm Credit New Producer Grant** | $15,000 | ⭐ Beginning farmer; reuses VAPG plan | 2026 closed May 31 · **watch ~spring 2027** · verify Catalina in AgWest territory |
| ☐ | **Tory Burch Fellows** | $5,000 + program | Ashley (women) | Annual · currently closed, watch |
| ☐ | **National Young Farmers Coalition grant** | $5,000 | Beginning farmer, any use | Verify "young" age rule · youngfarmers.org |

## 💵 CAPITAL TOOLS (not grants — cheap money if you need equipment fast)

| Status | Source | Amount | Terms |
|---|---|---|---|
| ☐ | **Kiva** | up to $15,000 | 0% interest, crowdfunded |
| ☐ | **Whole Foods Local Producer Loan** | up to $100,000 | Low-interest; WFM also a potential buyer |
| ☐ | **AZ Microbusiness Loan Program** | micro | State small-business loans |

## ❓ CHECK ELIGIBILITY

| Status | Grant | Amount | Catch to verify |
|---|---|---|---|
| ☐ | **Comcast RISE** | $5,000 + marketing | Ashley qualifies (woman-owned, 3+ yrs) — but confirm **Tucson is in Comcast service area** (may be Cox) |
| ☐ | **Community Foundation of S. AZ** | varies | Local, but mostly funds **nonprofits** — courtesy call only (520-770-0800) |

## 🛡️ Orchard safety net (set up at the FSA office — protection, not growth grants)
- **NAP** — cheap disaster coverage for your crop; enroll **before** a bad season.
- **Tree Assistance Program (TAP)** — 65% to replant trees lost to drought/freeze/
  fire blight; apply within 90 days of a loss.

## 🚫 Checked — NOT eligible (don't waste time)
- **HerRise Microgrant** — targets women of color; Ashley not eligible.
- **FedEx Small Business Grant** — program retired (ended 2024).
- **Farm Credit East AgEnhancement / Food Co-op Farmer Fund** — other states only.
- **Clif Family Foundation** — 501(c)(3) nonprofits only; for-profit farm not eligible.
- **Horizon Farm Credit YBS ($20k)** — mid-Atlantic territory only.

---

## 💰 Rough potential on the board
Quick wins (Amber, Galaxy, NASE, FruitGuys, AFT): **~$30k+ in reachable smaller grants**
Big swings (Business Builder $100k, VAPG, EQIP water): **six figures over time**
None guaranteed — grants are a numbers game — but this is a genuinely strong stack for a 1-acre beginning-farmer, woman-owned orchard.

## The rule
Keep applying. Amber resets every month. Losing is normal. Every "no" is one step
closer to a "yes," and each application reuses answers from the last.

## Details & drafts
- Application steps: `START_HERE_how_to_apply.md`
- New grants (full details): `additional_grants.md`
- Big grant drafts: `vapg/`
- Farm profile (private): `clients/howards_orchard.json`

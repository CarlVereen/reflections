...
- [ ] **A1.** New Google Sheet → `Extensions ▸ Apps Script`. Add the code files: `Code.gs` (paste over the stub), plus new **script** files `Api.gs`, `db.gs`, `setup.gs`, and new **HTML** files `WebApp`, `Sidebar` (optional: `Tests.gs`). **Save.** (Or `clasp push`, see `CLASP-SETUP.md`.)
- [ ] **A2.** Reload the sheet → a **⚡ CRM** menu appears → `⚡ CRM ▸ Set up / rebuild database` → approve the Google auth (Advanced → Go to project → Allow).
- [ ] **A3.** Confirm the **7 data tabs**: Clients, Jobs, Estimates, Invoices, LineItems, Services, Business Settings (plus a hidden `_meta`), with **no leftover "Sheet1."** The dashboard lives in the **app** (Home screen), not a sheet tab.
...
- [ ] **B2.** Open the URL → the **app loads on the Home screen** (KPI tiles, "Follow-ups due", "This week's jobs"). *(If you get a Drive "unable to open the file" error, you have multiple Google accounts, so open it in an Incognito window signed into just the owner account. `⚡ CRM ▸ 📲 Open the app (get link)` shows the URL anytime.)*
...
- [ ] **D1.** **Clients ▸ +** (or Home ▸ **+ Lead**) → add a client with **name, phone, email (= your own), and address** → **Save**. It appears in the **Clients** list (tagged **Lead**) **and** in the **Clients** sheet tab, with a **Next Follow-up** date set automatically.
...
- [ ] **G4.** Set an invoice's **Due Date = yesterday**, Status = **Sent** → `⚡ CRM ▸ Automations ▸ Flag overdue invoices now` → it flips to **Overdue**.
- [ ] **G5.** `⚡ CRM ▸ Automations ▸ Turn ON daily autopilot` → confirmation. `Apps Script ▸ Triggers` shows **4 time-based triggers**. Then **Turn OFF daily autopilot** → all removed.
...
## I. Google Calendar (now automatic, in the app)
Calendar sync is no longer a manual menu click. Scheduling a job **in the app** creates the event
automatically (client address as location), and rescheduling/cancelling updates or removes it. This
is covered in full by **Section O** below and by `runIntegrationSmoke` (`Tests.gs`).
- [ ] **I1.** In the app, add a job with a time like `2pm` (Calendar sync = On in Settings) → the event
  appears on your Google Calendar at 2pm. See Section O for the full lifecycle checks.
...
- [ ] **O2. First job → real event + permission.** With sync On, **Jobs ▸ + New job** for the test
  ...
  **Calendar**, approve it. Open your **Google Calendar**: an event appears on that date at **2pm**,
  titled with the **client name then the service**, with the client's **address as the location** (tap it → Maps).
